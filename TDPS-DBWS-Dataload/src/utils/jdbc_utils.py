...
def _build_sql_server_jdbc_url(jdbc_config, environment):
    sql_server = _resolve_env_setting(jdbc_config.get("server_by_env", jdbc_config.get("server")), environment, "jdbc.server_by_env")
    sql_database = _resolve_env_setting(jdbc_config.get("database_by_env", jdbc_config.get("database")), environment, "jdbc.database_by_env")
...
def _get_sql_server_access_token(source_config, environment, min_validity_seconds=SQL_TOKEN_MIN_VALIDITY_SECONDS):
    ...
    client_id = _require_value(_resolve_env_setting(auth_config.get("client_id_by_env", auth_config.get("client_id")), environment, "authentication.client_id_by_env"), "authentication.client_id_by_env")
    secret_scope = _resolve_env_setting(auth_config.get("secret_scope_by_env", auth_config.get("secret_scope")), environment, "authentication.secret_scope_by_env")
    ...
    client_secret = dbutils.secrets.get(scope=_require_value(secret_scope, "authentication.secret_scope_by_env"), key=secret_key)
...
def _get_sql_server_source_config(source_config, target_config):
    ...
    if not target_config:
        raise ValueError("SQL source requested jdbc.connection_source='target_config' but target_config_path was not provided or could not be loaded.")
    ...
    merged_jdbc_config = {
        ...
        "driver": jdbc_config.get("driver", target_connection.get("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver")),
...
def build_jdbc_options(source_config, environment, target_config=None):
    ...
    if database_type == "oracle":
        jdbc_url = _resolve_env_setting(jdbc_config.get("connection_by_env"), environment, "jdbc.connection_by_env")
        driver = jdbc_config.get("driver", "oracle.jdbc.driver.OracleDriver")
        user_options = dict(jdbc_config.get("options", {}))

        options = {
            "url": _require_value(jdbc_url, "jdbc.connection_by_env"),
            "driver": driver,
            "user": get_jdbc_username(source_config, environment),
            "password": get_jdbc_password(source_config, environment),
            "numPartitions": str(user_options.pop("numPartitions", 1)),
            "isolationLevel": "READ_COMMITTED",
            "oracle.net.encryption_client": "REQUIRED",
            "oracle.net.crypto_checksum_client": "REQUIRED",
            "oracle.jdbc.timezoneAsRegion": "false",
            "user.timezone": "UTC",
            "sessionTimeZone": user_options.pop("sessionTimeZone", "America/New_York")
        }
    ...
    raise ValueError(
        f"Unsupported database type '{database_type}'. Supported source database types are Oracle and SQL Server."
    )
